...
class Parser:
    ...
    def conector(self, line: str, the_graph: Graph) -> None:
        ...
        if zmax_link_capacity <= 0:
            raise ValueError(
                "max_link_capacity must be a positive integer bigger than 0"
            )
        if from_zone not in the_graph.zones or to_zone not in the_graph.zones:
            raise ValueError(
                f"Connection references unknown zone: {from_zone}-{to_zone}"
            )
